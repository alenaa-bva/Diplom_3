...
class BasePage:
    ...
    def wait_element_to_be_visible(self, driver, xpath, timeout=10):
        return WebDriverWait(driver, timeout).until(expected_conditions.visibility_of_element_located((By.XPATH, xpath)))

    def wait_element_not_visible(self, driver, xpath, timeout=10):
        try:
            # проверяем, существует ли элемент на странице
            WebDriverWait(driver, timeout).until_not(expected_conditions.visibility_of_element_located((By.XPATH, xpath)))
            return True
